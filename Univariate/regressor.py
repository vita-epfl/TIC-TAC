...
class Regressor(torch.nn.Module):
    """
    Defines the networks f and g, where y = f(x) and sigma ^2 = g(x)
    """
    def __init__(self, latent_dim: int, mu_blocks: int, var_blocks: int) -> None:
        super(Regressor, self).__init__()
        
        activation = torch.nn.Tanh

        # Network to predict the mean
        self.mu_model = [torch.nn.Linear(1, latent_dim), activation()]
        ...
        self.mu_model.append(torch.nn.Linear(latent_dim, 1))

        self.mu_length = len(self.mu_model)
        self.mu_model = torch.nn.ModuleList(self.mu_model)

        # Network to predict the variance
        self.var_model = [torch.nn.Linear(1, latent_dim), activation()]
        ...
        self.var_model.append(torch.nn.Linear(latent_dim, 3))
    ...
    def forward(self, x: torch.Tensor) -> (torch.Tensor, torch.Tensor):
        ...
        x_var = x_var ** 2    # three dimensional 
