...
class PoseNet(nn.Module):
    def __init__(self, arch):
        ...
        self.hgs = nn.ModuleList(
            [nn.Sequential(Hourglass(n=6, f=inp_dim, bn=False, increase=0)
        ...
        self.merge_features = nn.ModuleList(
            [Merge(inp_dim, inp_dim).cuda(cuda_devices[i]) for i in range(self.nstack-1)])
        self.merge_preds = nn.ModuleList(
            [Merge(oup_dim, inp_dim).cuda(cuda_devices[i]) for i in range(self.nstack-1)])

        self.global_avg_pool = nn.ModuleList(
            [nn.AvgPool2d(kernel_size=(64, 64), stride=1).cuda(cuda_devices[i]) for i in range(self.nstack)])
    ...
    def forward(self, imgs):
        ...
        for i in range(self.nstack):
            ...
            hourglass_dict['feature_7'] = x.clone().detach().to(
        ...
        if self.nstack == 1:
            return combined_hm_preds[0].unsqueeze(1), hourglass_dict
